  ...
  <div class="activation-page">
    <div class="card">
      ...
      <div v-if="currentScreen === 'start'" class="screen screen-start">
        <h1 class="title">🎓 أهلاً بك في إختبار بكالوريا انكليزي</h1>
      ...
      <div v-else-if="currentScreen === 'activation'" class="screen screen-activation">
        ...
        <p class="subtitle">
          انسخ رقم جهازك وأرسله لنا عبر الواتساب او الماسنجر ، ثم أدخل كود التفعيل الخاص بجهازك بعد ارساله إليك
        </p>

        <!-- رقم الجهاز مع Tooltip -->
        <div class="section tooltip-wrapper">
          <label class="label">الرقم الخاص بجهازي</label>
          <div class="device-code">{{ deviceId }}</div>
          <button
      :class="['btn copy-btn', { copied }]"
      @click="copyDeviceId"
    >
      {{ copied ? '✅ تم نسخ الرقم الخاص بجهازي وأصبح جاهز للارسال إلينا لنرسل لك كود التفعيل الموافق  ' : '📋 نسخ الرقم الخاص بجهازي' }}
    </button>
          <div class="tooltip">
            انقر على زر نسخ الرقم  ليتم نسخه  تلقائيًا ويصبح جاهزًا للإرسال الينا عبر الواتساب او الماسنجر
          </div>
        </div>

        <!-- أزرار التواصل -->
        <div class="section">
          <label class="label">أرسل الرقم المنسوخ لنا عبر الواتساب أو الماسنجر </label>
    ...
    <div v-if="showHelp" class="help-modal" @click.self="showHelp = false">
      <div class="help-box">
        ...
        <ul>
          ...
          <li>3️⃣ أدخل كود التفعيل  واضغط “تفعيل”.</li>
        </ul>
